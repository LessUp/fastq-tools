...
| 主张类型 | 文档可以怎样表述 | 去哪里复核 |
| --- | --- | --- |
| 产品边界 | FastQTools 当前维护 `stat`、`filter`、gzip 感知 FASTQ I/O、受控 predicate、受控 mutator 与可复用 C++ 表面。 | 产品规范与 API 页面 |
| 执行模型 | 吞吐解释必须回到连续批次所有权、`std::string_view` 记录视图、分阶段执行与 oneTBB 编排。 | 架构与算法章节 |
| 证据姿态 | 公开数字是带条件的代表性 benchmark 证据，不是可以直接搬运到任意机器或任意数据集的保证。 | 性能章节与 benchmark RFC |
| 研究语境 | 同类工具是范围与证据风格的对照锚点，不是 FastQTools 要全部取代的对象。 | 研究附录与书目 |

这张矩阵刻意保守。它防止文档滑向无法复核的宣传，同时仍然给高级读者足够材料去判断设计质量、运行适配度与比较语境。
...
第一遍读完之后，最稳妥的方式是把本站当成一份 dossier：

1. 先从本页收集维护中的范围；
2. 再到 [`架构`](../architecture/) 审查系统边界；
3. 再到 [`算法`](../algorithms/) 审查行为与不变量；
4. 再到 [`性能总览`](../performance/) 解读 benchmark 包络；
5. 只有当主系统叙事已经清楚时，再进入 [`研究附录`](../research/) 做引用与比较。
