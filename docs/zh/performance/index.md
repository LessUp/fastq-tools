...
建议把 headline 数字看成维护中的 benchmark 包络，而不是可以脱离上下文搬运的普适常数。这个包络同时包含硬件级别、构建模式、工作负载形状，以及“代表性发布样本”而非发布 SLA 的解释边界。
...
在站外引用任何数字之前，先套用这几条规则：

| 规则 | 结果 |
| --- | --- |
| 数字必须带上工作负载。 | 如果没有 read 数量、read 长度、构建模式与机器级别，吞吐数字就不可复核。 |
| 读、写、过滤、统计路径要分开看。 | 读入路径很强，并不自动证明整条 QC 工作流都同样强。 |
| 把压缩当成工作负载的一部分。 | gzip 成本会因为输入与存储条件不同而放大或遮蔽解析器行为。 |
| 采用决策必须本地复现。 | 公开快照是评估起点，不是采购保证。 |
...
因此，更好的问题通常不是“1696 MB/s 是不是绝对真理”，而是“项目是否给出了值得进一步评估的结果、方法和维护政策”。

## 风险边界

真正进入迁移、采购或 SLA 判断时，必须自己复现实验，并回到政策文件核对口径，而不是直接复述一个代表性数字。

## 复现实验路径

当原始指标已经读清楚后，可以沿着这条维护中的路径继续验证：

- [`Benchmark 报告`](./benchmark-report) —— 查看代表性公开快照；
- [`Benchmark 指南`](../dev/benchmark-guide) —— 复现实验环境与命令；
- [`研究附录 / 书目`](../research/bibliography) —— 核对术语、格式与引用来源；
- [`演进说明`](../research/evolution-notes) —— 理解 benchmark 政策为何被提升为一等设计材料。

## 不要只看 benchmark 报告

再用研究层补足上下文，避免把数字孤立解读：
