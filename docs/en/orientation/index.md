---
title: Orientation
description: Build the reading frame first, then move into the FastQTools whitepaper, academy, and reference materials.
layout: doc
---

<SectionLandingGrid locale="en" section="orientation" />

# Orientation

## Project position

FastQTools is an engineering-focused toolkit for the FASTQ QC core, not a workflow platform for every downstream bioinformatics job.

## Reading path

1. Start with [`Why FastQTools`](../why-fastqtools/)
2. Continue to [`Whitepaper`](../whitepaper/)
3. Move into [`Academy`](../academy/) and [`Reference`](../reference/)

## Terms

- `stat`: evidence-building and quality summaries
- `filter`: filtering, trimming, and preprocessing

## Next step

If you are still deciding whether the project is worth trying, continue to [`Why FastQTools`](../why-fastqtools/). If you want the full narrative spine right away, go straight to the [`Whitepaper`](../whitepaper/).
