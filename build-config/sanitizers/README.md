...
```bash
# Example: build-config/sanitizers/tsan.supp
race:third_party_library
deadlock:known_safe_pattern

# Example: build-config/sanitizers/lsan.supp (used with ASan)
...
```bash
export TSAN_OPTIONS="suppressions=build-config/sanitizers/tsan.supp:$TSAN_OPTIONS"
export LSAN_OPTIONS="suppressions=build-config/sanitizers/lsan.supp"
```
